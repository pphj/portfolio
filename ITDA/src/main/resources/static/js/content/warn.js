$(document).ready(function(){
    ...
    $('.btn_submit').on('click', function(e){
        ...
        let replyNum = $.urlParam('replyNum') || '';
        //let chNum = $.urlParam('chnum') || '';        // 수정
        let warnReason = '';
        ...
        $.ajax({
            type: "GET",
            url: "/itda/contents/checkReportedCategory",  // 이 URL은 실제 서버의 라우트에 맞게 변경해주세요.
            data: {
                userId: loginUser,
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status === "success") {
                                alert("신고 접수 성공");
                                //window.location.href = "/itda/contents/" + chnum + "/" + boardNum + "?userid=" + userid;  // 페이지를 리다이렉트합니다.
